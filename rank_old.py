...
def score_candidate(candi, data):
    ...
    for section, result in retrieval_results.items():
        for category, candis in result.items():
            for cid, distances in candis.items():
                if cid not in retrieved_candidates:
                    retrieved_candidates[cid] = {}
                if category not in retrieved_candidates[cid]:
                    retrieved_candidates[cid][category] = {}
                if section not in retrieved_candidates[cid][category]:
                    retrieved_candidates[cid][category][section] = []
                retrieved_candidates[cid][category][section].extend(distances)

    # Initial ranking --------------------------------------------------------------
    candi_scores = dict(
        sorted(
            (
                (cid, score_candidate(candidates[cid], data))
                for cid, data in retrieved_candidates.items()
            ),
            key=lambda x: x[1],
        )
    )
